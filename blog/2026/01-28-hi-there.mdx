---
title: Hi there!
description: This is the first post of my blog about my game development journey under the "Problem 18" brand.
url: https://github.com/problem-18-dev
image_url: https://avatars.githubusercontent.com/u/253351648
slug: test-blog
authors: problem-18-dev
tags:
  - personal
  - announcement
---

Being a software engineer by day and a gamer by life, game development has always been on my mind. Despite multiple very small attempts, I've never really put much effort into it. Problem 18 is my public journey to finally turn this interest into a skill, one game and 18 days at a time.

{/* truncate */}

### Who am I?

To start off, here's a quick gist of about me:

- I'm 26 years old. (This is easy to know by looking at what year we are!)
- I'm Belgian.
- I work as a software engineer by day.
- I have essentially no game development experience aside from **very** basic tutorials over years.

### What is Problem 18?

Problem 18 is my idea to publicize my journey to learn game development and to transform it into a strong hobby, with the intent to later move on and make a game of my own.

With my confidence in coding, I knew that I already have an advantage when it comes to creating basic games. I took a few looks through Reddit and other social medias to find out that the [20 games challenge](https://20_games_challenge.gitlab.io/ 'Link to the 20 games challenge website') is a popular way of getting started with game development for people who already have got the learning of programming out of the way.

So I told myself that if I want to finally make games instead of dreaming about it, this challenge is one hell of a way to start doing so.

#### Why do it publicly?

Because I know myself, and I realize that I quickly find loneliness during exploration of new hobbies, skills and interests.

Additionally, I would like to earn my place and belong in the game development community, and by keeping my games, my learnings and mistakes honest in public, I hope to do just that.

I even created a whole brand design, including this header. :-)

<img
  src='img/games/problem18-banner.jpg'
  width='100%'
  alt='Problem 18 banner'
/>

### Why "18"?

To keep myself in some discipline, as well as to keep the show going consistently, I decided that creating a game (and following the challenge) every 18 days would be a reasonable task. The number 18 was chosen for no particular reason other than my guess for how long on aware every game would take, considering I'm starting from zero.

I am aware that as I move up, the difficulty and scope of each game increases and we'll step beyond those 18 days sooner or later. Not to mention that I am learning many aspects on the go, and aim to have at the very least basic knowledge and skills in all of them by the time the challenge is done.

I've definitely got my work cut out for me, but I am not scared of it. In fact, I am so excited to be learning all these new skills, to get my creation out there, even if it just to a few people.

### Games

The games you can expect will be clones of cult classic games, starting from Flappy Bird, ending with Minecraft. On each game, I plan to add a small twist of my own. Whether that's the theme, some new mechanic or mood. Every game is taken from the 20 games challenge.

The plan as of today, followed by planned release date:

- Flappy Bird (January 18, 2026)
- Breakout (February 5, 2026)
- Frogger (February 23, 2026)
- Asteroids (March 13, 2026)
- Pac-man (March 31, 2026)
- Super Mario Bros (April 18, 2026)
- Worms (May 6, 2026)
- Crash Bandicoot (May 24, 2026)
- Doom (June 11, 2026)
- Minecraft (June 30, 2026)

So yeah, lots of work ahead of me. However, as of the day I'm writing this, I am semi-finished with Frogger and it's basically a month before the deadline. The game I'm most proud of right now is Keybricks.

<img
  src='img/games/keybricks_logo.png'
  width='50%'
  alt='Logo of game keybricks'
/>

#### Semi-finished?

I'm not too happy with how the game turned out. More about that in a future post.

### Skills

To make a game, one must develop all kinds of skills, I probably cannot even list them all yet as odds are I haven't even stumbled upon them.

**The goal is to be able to make games that present my visions and ideas, that will require me to have at the very least, strong basic knowledge in many skills.**

This includes:

- Art
- Music and sound effects
- Coding
- Game design
- UX and UI
- Whatever else games require!

I plan to use various tools to help me along the way. [Aseprite](https://www.aseprite.org/ 'Link to Aseprite') for 2D, [Blender](https://www.blender.org/) for 3D, [Audacity](https://www.audacityteam.org/) for SFX, [Godot](https://godotengine.org/) as the game engine, ..

### Thank you

So that's more or less what I can tell you about me and Problem 18 so far. I aim to keep these posts coming every second or third week, focusing on what I've learned, what mistakes I made, what obstacles I stumbled upon, and announcing the games, of course.

**If you're also into indie game development, whether you're a player or developer yourself, I would love to connect and learn about your adventures as well. Feel free to reach out on [X](https://x.com/Problem18Dev) or [Bluesky](https://bsky.app/profile/problem18dev.bsky.social), or check my games out on [Itch.io](https://problem-18.itch.io/)!**

Thank you for stopping by, and I hope that my learnings can teach you something at the same time, or at the very least you can have a laugh at my possibly very rookie mistakes.

<img
  src='img/games/problem_18_with_animation.png'
  width='60%'
  alt='problem 18 logo with stick figure symboling peace'
/>
